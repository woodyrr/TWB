<script setup>
    ...
    import mangabase from '../components/MangaBase.vue'
    const search_query = ref("");
    const mangalist = ref([]);
    const baseUrl = "https://api.jikan.moe/v4/anime";
    const baseUrl2 = "https://api.jikan.moe/v4/manga";
    const handle_search = async () => {
      mangalist.value = await fetch(`${baseUrl2}?q=${search_query.value}&limit=13`)
        .then(res => res.json())
        .then(data => data.data);
        search_query.value = ''
...
<template>
    <navigation />
    <!-- <div class="pt-20 "> -->
    <main class="text-center flex flex-col items-center justify-center gap-10 pt-20">
      <h1 class=" text-5xl head">THREEB</h1>
      <form class="w-full px-[12%] lg:px-[35%]"  @submit.prevent="handle_search">
        <input type="search" placeholder="Search for a Manga or Manhwa..." class="border border-gray-300 rounded-lg px-4 py-3 text-lg w-full outline-none drop-shadow-lg bg-gray-100" v-model="search_query"/>
      </form>
    </main>
    <div  v-if="mangalist.length > 0" class="px-[4%] xl:px-[8%] gap-10 lg:gap-6  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 pt-20 content-center">
      <div v-for="manga in mangalist" :key="manga.mal_id" :manga="manga" >
			  <div class="flex  gap-3 text-lg font-bold w-full items-center bg-gray-200 rounded-lg drop-shadow-lg" >
          
          <img :src="manga.images.jpg.large_image_url" :alt="manga.title +' poster'" class="h-[200px] md:h-[200px] lg:h-[250px] xl:h-[300px] w-[200px] md:w-[250px] lg:w-[200px]  border border-black rounded-l-lg" />
				  <div class="space-y-3 text-center justify-center">
            <h3>{{ manga.title }}</h3>
            <button  @click.prevent="addToHome(manga), toasts = true,addTotost(manga), addToHome2(manga) " class="bg-red-200 p-2  transition duration-300 ease-in-out hover:opacity-100 text-sm md:text-base rounded-lg">add to home</button>
          </div>
    ...
    <div class="flex flex-col gap-10 pt-20" v-else>
        ...
        <div class="px-[4%] xl:px-[8%] gap-10 lg:gap-6  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 pt-20 content-center">
          <Suspense>
            <template #default>
              <mangabase />
            </template>
